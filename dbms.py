...
conn = mysql.connector.connect(host="localhost",user="soham",password="2811",database="school_db",ssl_disabled=True)
...
while True:
	...
	if choice=='1':
		...
		if rows:
			...
			print('No employee with salary more than 50k')
	
	elif choice=='6':
		e_id=input('ID to search :')
		cursor.execute('select * from employee where rmpid=%s',(e_id,))
		row=cursor.fetchone()
